...
route.post('/signup', upload.single('image'), async (req, res) => {
    try {
        const { name, email, mobileNo, password, isPolice, policeIdName, policeIdNumber } = req.body;
    ...
    try {
        ...
        const newUser = user.map(users => {
            ...
            return {
                ...users.toObject(),
                image: imageUrls,
                isPolice: users?.isPolice || false,
                policeIdName: users?.policeIdName || "",
                policeIdNumber: users?.policeIdNumber || ""
            };
        })
...
route.patch('/profile/update', authMiddleWare, upload.single('image'), async (req, res) => {
    const userId = req.user.userId;
    const { name, email, mobileNumber, isPolice, policeIdName, policeIdNumber } = req.body;
